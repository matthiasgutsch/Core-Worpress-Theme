...
jQuery(document).ready(function($) {
    ...
    if ($('#navigation a').length != 0) {
        ...
        $('<div id="mobile_nav"><a id="mobile_nav_launch" class="visible_phone">Menu</a><div class="mobile_nav_wrap"></div></div>').insertAfter("#header");

        // Duplicate main menu for mobile version
        $( "ul#menu-primary-navigation" ).clone().appendTo( ".mobile_nav_wrap" );
        resMenu_cleanElement( $(".mobile_nav_wrap ul") );

        // Remove classes and ids of each li
        $(".mobile_nav_wrap ul li").each(function() {
            resMenu_cleanElement( $(this) );
        });

        // Add Down Caret
        $(".mobile_nav_wrap > ul > li > a").each(function() {
            if( $(this).parent().find('ul').length != 0 ) {
                $('<span class="mobile_nav_down_more"><i class="fa fa-caret-down"></i></span>').appendTo(this);
            }
        });

        // Onclick of down caret
        $(".mobile_nav_wrap span.mobile_nav_down_more").click(function(){
            $(this).closest('li').children('ul').slideToggle('fast');
            return false;
        });

        /* Controls the launch of the menu */
        $('#mobile_nav #mobile_nav_launch').click(function(){

            // Slide left le old navigation
            $('#mobile_nav .mobile_nav_wrap').toggleClass('mobile_nav_wrap_active');

            // Active class to button
            $('#mobile_nav #mobile_nav_launch').toggleClass('active_mobile_launch');

            // Slide left le html
            $('html').toggleClass('mobile_nav_is_active');
        });
    }
});

/* Removes extra styling */
function resMenu_cleanElement ( this_element ) {
    this_element.removeAttr('class');
    this_element.removeAttr('id');
}
